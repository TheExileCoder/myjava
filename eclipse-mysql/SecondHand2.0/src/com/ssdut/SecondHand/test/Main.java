...
public class Main {
		...
		while(flag) {
			...
			switch(type) {
				...
				case 2:
					...
					break;
				case 3:
					Main.ManagerGetAllCustomer(manager);
					flag = false;
					break;
				case 4:
					Main.ManagerGetOneCustomer(manager);
					flag = false;
					break;
				case 5:
					Main.ManagerUpdateCustomerPassWord(manager);
					flag = false;
					break;
				case 6:
					Main.ManagerDelCustomer(manager);
					flag = false;
					break;
				case 7:
					Main.ManagerAddCustomer(manager);
					flag = false;
					break;
				case 8:
	...
	/**
	 * @Description TODO 所有顾客的基本信息
	 * @param manager 已登录的管理员
	 * @return void
	 * @throws
	 */
	private static void ManagerGetAllCustomer(Manager manager) {
		ManagerService managerService = new ManagerServiceImpl();
	...
	/**
	 * @Description TODO 某个顾客的基本信息
	 * @param manager 已登录的管理员
	 * @return void
	 * @throws
	 */
	private static void ManagerGetOneCustomer(Manager manager) {
		ManagerService managerService = new ManagerServiceImpl();
		Customer customer = null;
	...
	/**
	 * @Description TODO 修改某个顾客的登陆密码
	 * @param manager 已登录的管理员
	 * @return void
	 * @throws
	 */
	private static void ManagerUpdateCustomerPassWord(Manager manager) {
		ManagerService managerService = new ManagerServiceImpl();
		managerService.updateCustomerPassWord();
		IsManagerLogOut(manager);
	}
	/**
	 * @Description TODO 删除顾客
	 * @param manager 已登录的管理员
	 * @return void
	 * @throws
	 */
	private static void ManagerDelCustomer(Manager manager) {
		ManagerService managerService = new ManagerServiceImpl();
	 ...
	 * @throws
	 */
	private static void ManagerAddCustomer(Manager manager) {
		ManagerService managerService = new ManagerServiceImpl();
		...
		while(flag) {
			...
			switch(type) {
				...
				case 4:
					...
					break;
				case 5:
					Main.CustomerAddGoods(customer);
					flag = false;
					break;
				case 6:
					Main.CustomerGoodsCommon(customer);
					flag = false;
					break;
				default:
					System.out.println("输入有误，请重新输入。");
	...
	private static void CustomerAddGoods(Customer customer) {
		CustomerService customerService = new CustomerServiceImpl();
		customerService.addGoods();
		IsCustomerLogOut(customer);
	}

	private static void CustomerGoodsCommon(Customer customer) {
		CustomerService customerService = new CustomerServiceImpl();
		customerService.Goodscommon(customer);
		IsCustomerLogOut(customer);
	}
	...
	private static void CustomerBuyGoods (Customer customer) {
		...
		CustomerService customerService = new CustomerServiceImpl();
		int printflag[] = {0};
		totalpay += customerService.buyGoods(customer, printflag);
		while(flag) {
			if(printflag[0] == 1) {
				CustomerService customerService1 = new CustomerServiceImpl();
				List<Goods> goodsList = new ArrayList<Goods>();
				goodsList = customerService1.getAllGoods();
				Goods goods = null;
				System.out.println("编号\t名称\t数量\t价格");
				for(int i = 0; i < goodsList.size(); i++) {
					goods = goodsList.get(i);
